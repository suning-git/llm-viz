...
then export the weights to `gpt-nano-toylang-model.json` in the *exact same schema*
as bbycroft's original (same keys / shapes / dtype / little-endian-float32 base64).

Adapted from train_2head.py.  Only the DATA GENERATOR and the acceptance eval change;
model config, init, optimizer, JSON export are identical.
...
"""
import argparse, base64, copy, json, math, pickle, time
...
class ToyLangDataset(Dataset):
    ...
    11 positions supervised).  Deterministic train/test split via hash of the sequence
    (minGPT demo convention: h % 4 == 0 -> test)."""
...
def main():
    ...
    ap.add_argument("--out", default="gpt-nano-toylang-model.json")
